...
```
src/
├── store/gameStore.ts   # Zustand store - all game state and actions
├── types/game.ts        # TypeScript types (GameSession, Player, Team, etc.)
├── pages/               # Route components (SetupPage, WordsPage, TeamsPage, PlayPage, etc.)
├── components/ui/       # Reusable UI components (Button, Card, Timer, Modal, etc.)
└── lib/utils.ts         # Utility functions (generateId, shuffleArray, formatTime, etc.)
```

### Game Flow

The game progresses through these statuses (defined in `GameStatus` type):
1. `words` - Players enter their words (WordsPage)
2. `teams` - Players are assigned to Team A or Team B (TeamsPage)
3. `playing` - Active gameplay with timer-based turns (PlayPage)
4. `phase-summary` - Summary between phases (PhaseSummaryPage)
5. `game-over` - Final results (GameOverPage)

### State Management

All game state lives in `useGameStore` (Zustand). Key patterns:
- State is persisted to localStorage under key `timesup-game-storage`
- Actions modify `session: GameSession | null` and `currentPlayerId`
- Use provided getters like `getCurrentWord()`, `getActivePlayer()`, `getTeamPlayers(team)`

### UI Components

Components are exported from `src/components/ui/index.ts`. The Card component supports `variant` prop for team-colored variants (`team-a`, `team-b`, `highlighted`).
